...
import { populateRarityNav, populateCatGrid, populateCatList, showErrorToast, populatePanelSummary } from './catalog_ui.js';
import { setupEventListeners } from './catalog_events.js';
import { initApi } from './catalog_api.js';
...
.then(([catsData, itemsData]) => {
    ...
    populateRarityNav();
    populatePanelSummary();
    populateCatGrid(); // Render the initial grid
    setupEventListeners();
    
    // Set the default rarity from the new dropdown
    const raritySelect = document.getElementById('rarity-select');
    if (raritySelect) {
        appState.currentRarity = raritySelect.value;
    }
    
    // Manually call refreshView() to show the initial cats
    refreshView();
...
function getFilteredAndSortedCats() {
    ...
    const selectedCheckboxes = Array.from(document.querySelectorAll('#advanced-filters input[type="checkbox"]:checked')).map(cb => cb.value);
    const ownershipFilter = document.querySelector('#advanced-filters input[name="ownership"]:checked')?.value || 'all';
