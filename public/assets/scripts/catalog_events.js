...
import { refreshView, updateBulkActionBar, setView } from './catalog_app.js'; 
import { isQuickEditMode } from './helpers.js';
import { populateCatGrid, populateCatList, showErrorToast, refreshCatElement } from './catalog_ui.js';
import { updateCatNumericProperty, updateCatForm, removeCat } from './catalog_api.js';
...
function clearAllSelections() {
    ...
    // ADD THIS LINE: Hide the detail panel content
    document.getElementById('detail-panel-card')?.classList.add('hidden'); // Target the card
    // Also clear the current cat in state
...
export function setupEventListeners() {
    ...
    const rightPanel = document.querySelector('.catalog-detail-panel');
    ...
    if (toggleFiltersBtn && advancedFilters) {
        toggleFiltersBtn.addEventListener('click', () => {
            ...
            toggleFiltersBtn.textContent = isHidden ? 'Filters ▼' : 'Filters ▲';
        });
    }
    ...
    if (raritySelect) {
        raritySelect.addEventListener('change', (e) => {
            clearAllSelections();
            appState.currentRarity = e.target.value;
            refreshView();
        });
    }
    ...
    document.getElementById('advanced-filters').addEventListener('change', () => {
        clearAllSelections(); // <-- ADD THIS
        refreshView();
    });
    document.getElementById('clear-filters-btn').addEventListener('click', () => {
        ...
        document.querySelectorAll('#advanced-filters input[type="checkbox"]').forEach(checkbox => { checkbox.checked = false; });
        const allRadio = document.querySelector('#advanced-filters input[name="ownership"][value="all"]');
