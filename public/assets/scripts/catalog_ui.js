...
import { onAddCat, updateCatNumericProperty, updateCatForm } from './catalog_api.js';
...
export function populateRarityNav() {
    ...
    // Add "All Rarities" at the end
    const allOption = document.createElement('option');
    allOption.value = 'all';
    allOption.textContent = 'All Rarities';
    select.appendChild(allOption);
    
    // Set default value to "Normal" (if it exists)
    if (rarities.includes('Normal')) {
        select.value = 'Normal';
    }
}
...
export function displayCatInDetailPanel(cat) {
    // Get the detail card element
    const detailCard = document.getElementById('detail-panel-card');

    // If clicking the *same* cat, hide the detail card.
    if (cat && appState.currentCat && cat.cat_id === appState.currentCat.cat_id) {
        ...
        detailCard.classList.add('hidden'); // Hide the card
        return;
    }
    ...
    if (!cat) {
        // No cat selected, hide detail card
        detailCard.classList.add('hidden');
        return;
    }
    
    // A cat IS selected, show its card and populate content
    detailCard.classList.remove('hidden'); // Show the card
    Logger.info(`Displaying details for: ${cat.cat_name}`, cat);
    
    // Highlight the selected element in the grid/list
    const element = catElementMap.get(cat.cat_id);
    if (element) element.classList.add('selected');

    // --- Content population remains the same (READ-ONLY) ---
    batchUpdateElements([
    ...
    cat.forms.forEach((form, index) => {
        // ... (all the form detail/requirement logic) ...
...
// Add this function somewhere in catalog_ui.js
export function populatePanelSummary() {
    Logger.info('Populating panel summary...');
    // --- Basic Stats Calculation ---
    const totalCats = appState.userCats.length;
    const ownedCats = appState.userCats.filter(cat => cat.user_info);
    const ownedCount = ownedCats.length;
    const missingCount = totalCats - ownedCount;
    const evolveCount = ownedCats.filter(cat => canCatEvolve(cat)).length; // Use helper

    // --- Update Stats DOM ---
    batchUpdateElements([
        { id: 'summary-owned-count', text: ownedCount },
        { id: 'summary-total-count', text: totalCats },
        { id: 'summary-evolve-count', text: evolveCount },
        { id: 'summary-missing-count', text: missingCount }
    ]);

    // --- Basic Item Display ---
    const itemsContainer = document.getElementById('summary-items');
    if (itemsContainer) {
        // Simple list for now - enhance later
        itemsContainer.innerHTML = '<ul>' + appState.userItems
            .filter(item => item.item_qty > 0) // Only show items the user has
            .slice(0, 10) // Limit for brevity
            .map(item => `<li>${item.item_name}: ${item.item_qty}</li>`)
            .join('') + '</ul>';
        // TODO: Improve item display (icons, required amounts?)
    }
}